...
sc.addEventListener("mousemove",function(e){if(!dragState||!srcImg)return;var r=sc.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;applyDrag(mx,my);needsRedraw=true;if(!dragState)return;sc.style.cursor=dragState.type==="pan"?"grabbing":(dragState.type==="move"?"grabbing":"nwse-resize");});
