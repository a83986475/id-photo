...
function getFixedCrop(){var W=sc.width,H=sc.height,ratio=getSpecRatio(),cw=Math.min(W*0.72,H*0.72*ratio),ch=cw/ratio,cx=(W-cw)/2,cy=(H-ch)/2;return{canvasX:cx,canvasY:cy,canvasW:cw,canvasH:ch,cx:vx+cx/W*vw,cy:vy+cy/H*vh,pw:cw/W*vw,ph:ch/H*vh};}
...
function initCropRect(bmp,det){
  ...
  if(det){
    var fc=getFixedCrop();var targetPw=fh/hsVal;var zoom=Math.max(0.5,Math.min(targetPw/fc.pw,3));
...
sc.addEventListener("mousedown",function(e){if(!srcImg||!cropRect||!(vw<srcImg.width||vh<srcImg.height))return;var r=sc.getBoundingClientRect();dragState={origin:{vx:vx,vy:vy},start:{x:e.clientX-r.left,y:e.clientY-r.top}};sc.style.cursor="grabbing";});
sc.addEventListener("mousemove",function(e){if(!dragState||!srcImg)return;var r=sc.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top,dx=mx-dragState.start.x,dy=my-dragState.start.y,iw=srcImg.width,ih=srcImg.height,scX=vw/sc.width,scY=vh/sc.height;vx=Math.max(0,Math.min(dragState.origin.vx-dx*scX,iw-vw));vy=Math.max(0,Math.min(dragState.origin.vy-dy*scY,ih-vh));needsRedraw=true;});
...
sc.addEventListener("touchstart",function(e){if(!srcImg||!cropRect||e.touches.length!==1||!(vw<srcImg.width||vh<srcImg.height))return;var p=getTouchPos(e);dragState={type:"pan",origin:{vx:vx,vy:vy},start:{x:p.x,y:p.y}};e.preventDefault();},{passive:false});
sc.addEventListener("touchmove",function(e){if(!dragState||!srcImg||e.touches.length!==1)return;e.preventDefault();var p=getTouchPos(e),dx=p.x-dragState.start.x,dy=p.y-dragState.start.y,iw=srcImg.width,ih=srcImg.height,scX=vw/sc.width,scY=vh/sc.height;vx=Math.max(0,Math.min(dragState.origin.vx-dx*scX,iw-vw));vy=Math.max(0,Math.min(dragState.origin.vy-dy*scY,ih-vh));needsRedraw=true;},{passive:false});
