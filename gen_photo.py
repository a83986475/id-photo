...
function applyDrag(mx,my){
  ...
  if(dragState.type==="pan"){vx=Math.max(0,Math.min(dragState.origin.vx-dx,iw-vw));vy=Math.max(0,Math.min(dragState.origin.vy-dy,ih-vh));return;}
...
sc.addEventListener("mousedown",function(e){if(!srcImg||!cropRect)return;var r=sc.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;var t=getDragTarget(mx,my);if(t){dragState=t;dragState.start.ix=cxToImg(mx);dragState.start.iy=cyToImg(my);sc.style.cursor=t.type==="move"?"grabbing":(t.type==="pan"?"grabbing":"nwse-resize");}else if(vw<srcImg.width||vh<srcImg.height){dragState={type:"pan",origin:{vx:vx,vy:vy},start:{ix:cxToImg(mx),iy:cyToImg(my)}};sc.style.cursor="grabbing";}});
sc.addEventListener("mousemove",function(e){if(!dragState||!srcImg)return;var r=sc.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;applyDrag(mx,my);needsRedraw=true;sc.style.cursor=dragState.type==="pan"?"grabbing":(dragState.type==="move"?"grabbing":"nwse-resize");});
...
sc.addEventListener("touchstart",function(e){if(!srcImg||!cropRect||e.touches.length!==1)return;var p=getTouchPos(e),mx=p.x,my=p.y;var t=getDragTarget(mx,my);if(t){dragState=t;dragState.start.ix=cxToImg(mx);dragState.start.iy=cyToImg(my);}else if(vw<srcImg.width||vh<srcImg.height){dragState={type:"pan",origin:{vx:vx,vy:vy},start:{ix:cxToImg(mx),iy:cyToImg(my)}};}e.preventDefault();},{passive:false});
sc.addEventListener("touchmove",function(e){if(!dragState||!srcImg||e.touches.length!==1)return;e.preventDefault();var p=getTouchPos(e),mx=p.x,my=p.y;applyDrag(mx,my);needsRedraw=true;},{passive:false});
