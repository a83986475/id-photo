...
function drawFaceGuide(ctx,det,W,H,iw,ih){
  var sw=W/vw,sh=H/vh;ctx.save();
  if(det&&det._sx&&cropRect){var sx=det._sx,sy=det._sy;var pts=det.landmarks.positions;var ly=0;for(var i=36;i<48;i++)ly+=pts[i].y;ly=ly/12*sy;ctx.strokeStyle="rgba(0,220,200,.55)";ctx.fillStyle="rgba(0,220,200,.15)";ctx.lineWidth=2.5;var eh=(pts[8].y-pts[27].y)*sy*2.0,ew=eh*0.85;var fcx=cropRect.fcx;ctx.beginPath();ctx.ellipse(imgToCx(fcx),imgToCy(ly),ew*sw/2,eh*sh/2,0,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="rgba(0,220,200,.4)";[37,46].forEach(function(i){var p=pts[i];ctx.beginPath();ctx.arc(imgToCx(p.x*sx),imgToCy(p.y*sy),5,0,Math.PI*2);ctx.fill();});ctx.strokeStyle="rgba(0,220,200,.35)";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(imgToCx(pts[27].x*sx),imgToCy(pts[27].y*sy));ctx.lineTo(imgToCx(pts[30].x*sx),imgToCy(pts[30].y*sy));ctx.stroke();}else{ctx.strokeStyle="rgba(0,220,200,.4)";ctx.fillStyle="rgba(0,220,200,.1)";ctx.lineWidth=2;var cx2=imgToCx(iw/2),cy2=imgToCy(ih*0.4),eh2=(ih*0.45)/vh*H,ew2=eh2*0.85;ctx.beginPath();ctx.ellipse(cx2,cy2,ew2/2,eh2/2,0,0,Math.PI*2);ctx.fill();ctx.stroke();}
  ctx.restore();
}
