...
function cropCanvas(bmp,det,comp,rect){var dims=sz.value.split("x").map(Number),tw=dims[0],th=dims[1],iw=bmp.width,ih=bmp.height,cx,cy,pw2,ph;if(rect&&!isNaN(rect.cx)&&rect.pw>0){cx=Math.max(0,Math.min(rect.cx,iw-10));cy=Math.max(0,Math.min(rect.cy,ih-10));pw2=Math.max(10,Math.min(rect.pw,iw-cx));ph=Math.max(10,Math.min(rect.ph,ih-cy));}else{var ratio=tw/th,hsVal=0.42,tm2=0.35,fcx=iw/2,fT=ih*0.12,fB=ih*0.5;if(det&&det._sx){var sx=det._sx,sy=det._sy,pts=det.landmarks.positions,lx=0,rx2=0;for(var i=36;i<42;i++)lx+=pts[i].x*sx;for(var i=42;i<48;i++)rx2+=pts[i].x*sx;fcx=(lx/6+rx2/6)/2;var b=det.detection.box;fT=b.y*sy;fB=(b.y+b.height)*sy;}else if(det){var pts=det.landmarks.positions,lx=0,rx2=0;for(var i=36;i<42;i++)lx+=pts[i].x;for(var i=42;i<48;i++)rx2+=pts[i].x;fcx=(lx/6+rx2/6)/2;var b=det.detection.box;fT=b.y;fB=b.y+b.height;}var fh=fB-fT,ph2=fh/hsVal;pw2=ph2*ratio;cx=fcx-pw2/2;cy=fT-ph2*tm2;cx=Math.max(0,Math.min(cx,iw-pw2));cy=Math.max(0,Math.min(cy,ih-ph2));ph=Math.min(ph2,ih-cy);pw2=Math.min(pw2,iw-cx);}var out=document.createElement("canvas");out.width=Math.max(1,Math.round(tw));out.height=Math.max(1,Math.round(th));try{out.getContext("2d").drawImage(comp,Math.round(cx),Math.round(cy),Math.round(pw2),Math.round(ph),0,0,out.width,out.height);}catch(e){out.getContext("2d").fillText("Error",10,20);}return out;}
