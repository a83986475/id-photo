...
function zoomReset(){if(!srcImg)return;vx=0;vy=0;vw=srcImg.width;vh=srcImg.height;needsRedraw=true;updateZoomLabel();}
document.getElementById("zoomOut").addEventListener("click",function(){zoomStep(-0.08);});
document.getElementById("zoomIn").addEventListener("click",function(){zoomStep(0.08);});
document.getElementById("zoomReset").addEventListener("click",zoomReset);
// Mousewheel zoom on crop canvas
sc.addEventListener("wheel",function(e){e.preventDefault();var d=e.deltaY>0?0.08:-0.08;zoomStep(d);},{passive:false});
...
function zoomStep(delta){if(!srcImg)return;var ctrX=vx+vw/2,ctrY=vy+vh/2;vw*=(1+delta);vh*=(1+delta);var minW=srcImg.width*0.1,minH=srcImg.height*0.1;if(vw<minW||vh<minH||vw>srcImg.width*3||vh>srcImg.height*3){vw/=(1+delta);vh/=(1+delta);return;}vx=ctrX-vw/2;vy=ctrY-vh/2;vx=Math.max(0,Math.min(vx,srcImg.width-vw));vy=Math.max(0,Math.min(vy,srcImg.height-vh));needsRedraw=true;updateZoomLabel();}
function updateZoomLabel(){var el=document.getElementById("zoomPct");if(el&&srcImg)el.textContent=Math.round(srcImg.width/vw*100)+"%";}
...
function getFixedCrop(){var W=sc.width,H=sc.height,ratio=getSpecRatio(),cw=Math.min(W*0.58,H*0.58*ratio),ch=cw/ratio,cx=(W-cw)/2,cy=(H-ch)/2;return{canvasX:cx,canvasY:cy,canvasW:cw,canvasH:ch,cx:vx+cx/W*vw,cy:vy+cy/H*vh,pw:cw/W*vw,ph:ch/H*vh};}
function startRenderLoop(){if(renderId)return;!function loop(){var isAnim=!!animFrom;if(isAnim){var t=Math.min(1,(Date.now()-animStart)/350),e=easeOut(t);vx=animFrom.vx+(animTo.vx-animFrom.vx)*e;vy=animFrom.vy+(animTo.vy-animFrom.vy)*e;if(t>=1){animFrom=null;}needsRedraw=true;}pulsePhase=(pulsePhase+0.035)%(Math.PI*2);if(needsRedraw){var fc=getFixedCrop();if(cropRect){cropRect.cx=fc.cx;cropRect.cy=fc.cy;cropRect.pw=fc.pw;cropRect.ph=fc.ph;}drawCropOverlay();updatePreview();needsRedraw=false;}renderId=requestAnimationFrame(loop);}();}
...
var th=window.matchMedia("(prefers-color-scheme:dark)").matches?"dark":"light";root.setAttribute("data-theme",th);tbtn.textContent=th==="dark"?"\u2600\ufe0f":"🌙";tbtn.addEventListener("click",function(){th=th==="dark"?"light":"dark";root.setAttribute("data-theme",th);this.textContent=th==="dark"?"\u2600\ufe0f":"🌙";});
...
function cropCanvas(bmp,det,comp,rect){var dims=sz.value.split("x").map(Number),tw=Math.max(1,parseInt(dims[0])||295),th=Math.max(1,parseInt(dims[1])||413),iw=bmp.width,ih=bmp.height,cx,cy,pw2,ph;if(rect&&!isNaN(rect.cx)&&rect.pw>0){if(isNaN(rect.ph)||rect.ph<=0)rect.ph=rect.pw/(tw/th);cx=Math.max(0,Math.min(rect.cx,iw-10));cy=Math.max(0,Math.min(rect.cy,ih-10));pw2=Math.max(10,Math.min(rect.pw,iw-cx));ph=Math.max(10,Math.min(rect.ph,ih-cy));}else{var ratio=tw/th,hsVal=0.42,tm2=0.35,fcx=iw/2,fT=ih*0.12,fB=ih*0.5;if(det&&det._sx){var sx=det._sx,sy=det._sy,pts=det.landmarks.positions,lx=0,rx2=0;for(var i=36;i<42;i++)lx+=pts[i].x*sx;for(var i=42;i<48;i++)rx2+=pts[i].x*sx;fcx=(lx/6+rx2/6)/2;var b=det.detection.box;fT=b.y*sy;fB=(b.y+b.height)*sy;}else if(det){var pts=det.landmarks.positions,lx=0,rx2=0;for(var i=36;i<42;i++)lx+=pts[i].x;for(var i=42;i<48;i++)rx2+=pts[i].x;fcx=(lx/6+rx2/6)/2;var b=det.detection.box;fT=b.y;fB=b.y+b.height;}var fh=fB-fT,ph2=fh/hsVal;pw2=ph2*ratio;cx=fcx-pw2/2;cy=fT-ph2*tm2;cx=Math.max(0,Math.min(cx,iw-pw2));cy=Math.max(0,Math.min(cy,ih-ph2));ph=Math.min(ph2,ih-cy);pw2=Math.min(pw2,iw-cx);}var out=document.createElement("canvas");out.width=tw;out.height=th;try{out.getContext("2d").drawImage(comp,Math.round(cx),Math.round(cy),Math.round(pw2),Math.round(ph),0,0,tw,th);}catch(e){out.getContext("2d").drawImage(bmp,0,0,tw,th);}return out;}
...
function initCropRect(bmp,det){
  ...
  if(det){
    var fc=getFixedCrop();var targetPw=fh/hsVal;var zoom=Math.max(1,Math.min(targetPw/fc.pw,3));
    vw=iw/zoom;vh=ih/zoom;
    vx=Math.max(0,Math.min(fcx-vw/2,iw-vw));vy=Math.max(0,Math.min((fT+fB)/2-vh*0.45,ih-vh));
    var fromVx=Math.max(0,Math.min(origW/2-vw/2,iw-vw));
    var fromVy=Math.max(0,Math.min(origH*0.3-vh*0.4,ih-vh));
    animFrom={vx:fromVx,vy:fromVy};animTo={vx:vx,vy:vy};animStart=Date.now();
  }
...
function drawFaceGuide(ctx,det,W,H,iw,ih){
  ...
  ctx.strokeStyle="rgba(255,200,50,.6)";ctx.lineWidth=2.5;ctx.setLineDash([5,7]);
  ctx.beginPath();ctx.moveTo(ccx+12,eyeY);ctx.lineTo(ccx+cpw-12,eyeY);ctx.stroke();ctx.setLineDash([]);
  // Eye level label
  ctx.fillStyle="rgba(255,200,50,.8)";ctx.font="bold 10px system-ui,sans-serif";ctx.textAlign="left";
  ctx.fillText("\u25c6 \u773c\u775b\u53c2\u8003\u7ebf",ccx+16,eyeY-5);
  ...
  if(cropRect.fh){
    ...
    grad.addColorStop(0,"rgba(0,255,180,.2)");
    grad.addColorStop(1,"rgba(0,255,180,.05)");
    ctx.fillStyle=grad;
    ctx.beginPath();ctx.ellipse(targetFcx,targetFcy,targetFw*0.5,targetFh*0.5,0,0,Math.PI*2);ctx.fill();
    // Ghost face outline
    ctx.strokeStyle="rgba(0,255,180,.55)";ctx.lineWidth=2;ctx.setLineDash([4,6]);
    ctx.beginPath();ctx.ellipse(targetFcx,targetFcy,targetFw*0.5,targetFh*0.5,0,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);
    // Chin dot
    ctx.fillStyle="rgba(0,255,180,.6)";ctx.beginPath();ctx.arc(targetFcx,targetFcy+targetFh*0.48,4,0,Math.PI*2);ctx.fill();
    // Top of head marker
    ctx.fillStyle="rgba(0,255,180,.5)";ctx.beginPath();ctx.arc(targetFcx,targetFcy-targetFh*0.48,4,0,Math.PI*2);ctx.fill();
    // Label
    ctx.fillStyle="rgba(0,255,180,.6)";ctx.font="bold 9px system-ui,sans-serif";ctx.textAlign="center";
    ctx.fillText("\u76ee\u6807\u8138\u90e8\u4f4d\u7f6e",targetFcx,targetFcy+targetFh*0.5+14);
  }
  
  // --- Detected face overlay (where face IS, vivid colors) ---
  if(det&&det._sx&&cropRect){var sx=det._sx,sy=det._sy;var pts=det.landmarks.positions;var ly=0;for(var i=36;i<48;i++)ly+=pts[i].y;ly=ly/12*sy;var eh=(pts[8].y-pts[27].y)*sy*2.4,ew=eh*0.82;var fcx=cropRect.fcx;
    ctx.strokeStyle="rgba(255,100,50,.6)";ctx.fillStyle="rgba(255,100,50,.2)";ctx.lineWidth=2.5;ctx.setLineDash([]);
    ctx.beginPath();ctx.ellipse(imgToCx(fcx),imgToCy(ly),ew*sw/2,eh*sh/2,0,0,Math.PI*2);ctx.fill();ctx.stroke();
    // Detected eye dots - bright yellow
    ctx.fillStyle="rgba(255,220,50,.9)";[37,46].forEach(function(i){var p=pts[i];ctx.beginPath();ctx.arc(imgToCx(p.x*sx),imgToCy(p.y*sy),5,0,Math.PI*2);ctx.fill();});
    // Chin dot
    ctx.fillStyle="rgba(255,100,50,.6)";ctx.beginPath();ctx.arc(imgToCx(pts[8].x*sx),imgToCy(pts[8].y*sy),4,0,Math.PI*2);ctx.fill();
    // Detected label
    ctx.fillStyle="rgba(255,100,50,.7)";ctx.font="bold 9px system-ui,sans-serif";ctx.textAlign="center";
    ctx.fillText("\u68c0\u6d4b\u5230\u7684\u8138",imgToCx(fcx),imgToCy(ly)-eh*sh*0.5-8);
  }else{
...
function drawCropOverlay(){
  ...
  var fc=getFixedCrop(),ccx=fc.canvasX,ccy=fc.canvasY,cpw=fc.canvasW,cph=fc.canvasH;
  ...
  var hsz=16;["nw","ne","sw","se"].forEach(function(c,i){var x=i%2===0?ccx:ccx+cpw-hsz,y=i<2?ccy:ccy+cph-hsz;sx.fillStyle="#fff";sx.fillRect(x,y,hsz,hsz);sx.strokeStyle="var(--p)";sx.lineWidth=2.5;sx.strokeRect(x,y,hsz,hsz);});
  ...
  if(faceRes&&faceRes._sx){var pts=faceRes.landmarks.positions;[36,39,42,45].forEach(function(i){var p=pts[i];var px=imgToCx(p.x*faceRes._sx),py=imgToCy(p.y*faceRes._sy);sx.fillStyle="rgba(255,200,50,.8)";sx.beginPath();sx.arc(px,py,5,0,Math.PI*2);sx.fill();});}
  ...
  if(cropRect.cx<vx||cropRect.cx+cropRect.pw>vx+vw||cropRect.cy<vy||cropRect.cy+cropRect.ph>vy+vh){sx.fillStyle="rgba(255,50,50,.12)";sx.fillRect(0,0,W,H);var warn=Math.max(0,250-250*(Date.now()%3000)/3000);sx.fillStyle="rgba(255,50,50,"+Math.min(1,warn/255)+")";sx.font="bold 14px system-ui,sans-serif";sx.textAlign="center";sx.fillText("\u26a0 \u88c1\u5207\u6846\u8d85\u51fa\u56fe\u7247\uff0c\u7f29\u5c0f\u67e5\u770b",W/2,40);sx.textAlign="start";}
}
...
function getTouchPos(e){var t=e.touches[0],r=sc.getBoundingClientRect();return{x:t.clientX-r.left,y:t.clientY-r.top};}
// Pan-only drag handlers (crop rect is fixed, only photo moves)
sc.addEventListener("mousedown",function(e){if(!srcImg||!cropRect)return;var r=sc.getBoundingClientRect();dragState={origin:{vx:vx,vy:vy},start:{x:e.clientX-r.left,y:e.clientY-r.top}};sc.style.cursor="grabbing";});
sc.addEventListener("mousemove",function(e){if(!dragState||!srcImg)return;var r=sc.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top,dx=mx-dragState.start.x,dy=my-dragState.start.y,iw=srcImg.width,ih=srcImg.height,scX=vw/sc.width,scY=vh/sc.height;vx=Math.max(Math.min(0,iw-vw)-50*vw/sc.width,Math.min(dragState.origin.vx-dx*scX,Math.max(0,iw-vw)+50*vw/sc.width));vy=Math.max(Math.min(0,ih-vh)-50*vh/sc.height,Math.min(dragState.origin.vy-dy*scY,Math.max(0,ih-vh)+50*vh/sc.height));needsRedraw=true;});
sc.addEventListener("mouseup",function(){if(dragState){sc.style.cursor="default";dragState=null;}});
sc.addEventListener("mouseleave",function(){if(dragState){sc.style.cursor="default";dragState=null;}});
// Touch pan handlers (mobile)
sc.addEventListener("touchstart",function(e){if(!srcImg||!cropRect||e.touches.length!==1)return;var p=getTouchPos(e);dragState={type:"pan",origin:{vx:vx,vy:vy},start:{x:p.x,y:p.y}};e.preventDefault();},{passive:false});
sc.addEventListener("touchmove",function(e){if(!dragState||!srcImg||e.touches.length!==1)return;e.preventDefault();var p=getTouchPos(e),dx=p.x-dragState.start.x,dy=p.y-dragState.start.y,iw=srcImg.width,ih=srcImg.height,scX=vw/sc.width,scY=vh/sc.height;vx=Math.max(Math.min(0,iw-vw)-50*vw/sc.width,Math.min(dragState.origin.vx-dx*scX,Math.max(0,iw-vw)+50*vw/sc.width));vy=Math.max(Math.min(0,ih-vh)-50*vh/sc.height,Math.min(dragState.origin.vy-dy*scY,Math.max(0,ih-vh)+50*vh/sc.height));needsRedraw=true;},{passive:false});
sc.addEventListener("touchend",function(e){if(dragState){dragState=null;needsRedraw=true;}e.preventDefault();},{passive:false});
function save_photos(canvas,fmt,dpi,qual){return new Promise(function(res,rej){var mt=fmt==="jpeg"?"image/jpeg":"image/png",q=fmt==="jpeg"?qual/100:undefined,timeout=setTimeout(function(){rej(new Error("Save timeout"));},15000);canvas.toBlob(function(b){clearTimeout(timeout);if(!b){rej(new Error("Blob creation failed"));return;}var sv=document.getElementById("sizeVal");if(sv)sv.textContent=formatSize(b.size);if(fmt==="png"){injectDPI(b,dpi).then(function(b2){if(sv)sv.textContent=formatSize(b2.size);res({blob:b2,ext:".png"});}).catch(function(){if(sv)sv.textContent=formatSize(b.size);res({blob:b,ext:".png"});});}else{injectJPEGDPI(b,dpi).then(function(b2){if(sv)sv.textContent=formatSize(b2.size);res({blob:b2,ext:".jpg"});}).catch(function(){if(sv)sv.textContent=formatSize(b.size);res({blob:b,ext:".jpg"});});}},mt,q);});}
async function handleFile(file){if(!file)return;cropRect=null;dragState=null;animFrom=null;animTo=null;cachedAl=null;cachedComp=null;dlBlob=null;vx=0;vy=0;vw=0;vh=0;needsRedraw=true;if(dlURL){URL.revokeObjectURL(dlURL);dlURL=null;}setStatus("\u6b63\u5728\u52a0\u8f7d\u56fe\u7247\u5e76\u68c0\u6d4b\u4eba\u8138\u2026");photoName.textContent=file.name;genBtn.disabled=true;dlBtn.disabled=true;if(dlURL){URL.revokeObjectURL(dlURL);dlURL=null;}srcImg=await createImageBitmap(file);photoDims.textContent=srcImg.width+"x"+srcImg.height+"px";photoInfo.classList.remove("hidden");fitDraw(sc,sx,srcImg);faceRes=await detectFace(sc);if(faceRes){var _sx=srcImg.width/sc.width,_sy=srcImg.height/sc.height;faceRes._sx=_sx;faceRes._sy=_sy;}vx=0;vy=0;vw=srcImg.width;vh=srcImg.height;initCropRect(srcImg,faceRes);drawCropOverlay();drawLM(sx,faceRes);fn.textContent=faceRes?"\u2713 \u68c0\u6d4b\u5230\u4eba\u8138\uff0c\u9884\u8bbe\u88c1\u5207\u6846\u5df2\u81ea\u52a8\u5c45\u4e2d":"\u26a0 \u672a\u68c0\u6d4b\u5230\uff0c\u88c1\u5207\u6846\u5c45\u4e2d\u4f30\u7b97\u3002\u53ef\u62d6\u62fd\u8c03\u6574";drawMask(null);mn.textContent="\u70b9\u51fb\u300c\u751f\u6210\u8bc1\u4ef6\u7167\u300d\u540e\u83b7\u53d6\u62a0\u56fe";rn.textContent="\u62d6\u62fd\u88c1\u5207\u6846\u9884\u89c8\u6548\u679c";rc.width=0;rc.height=0;cachedAl=null;cachedComp=null;  var bt=document.querySelector("input[name=bg]:checked").value;if(bt!=="original"&&!rmbgSess)rw.classList.remove("hidden");genBtn.disabled=false;setStep(2);setStatus(faceRes?"\u2713 \u5df2\u5c31\u7eea\uff01\u62d6\u62fd\u88c1\u5207\u6846\u8c03\u6574\u4f4d\u7f6e\uff0c\u7136\u540e\u70b9\u51fb\u751f\u6210\u3002":"\u5df2\u52a0\u8f7d\uff0c\u53ef\u62d6\u62fd\u88c1\u5207\u6846\u8c03\u6574\u4f4d\u7f6e\u3002",faceRes?"ok":"");showToast("\u7167\u7247\u5df2\u52a0\u8f7d ("+srcImg.width+"\u00d7"+srcImg.height+")");
  document.getElementById("zoomCtrl").classList.remove("hidden");updateZoomLabel();
}
...
async function genClick(){
  ...
  var comp,single;
  try{
    comp=composite(srcImg,al);cachedComp=comp;
    single=cropCanvas(srcImg,faceRes,comp,cropRect);
  }catch(e){setStatus("\u88c1\u5207\u51fa\u9519\uff0c\u8bd5\u7528\u539f\u56fe\u76f4\u63a5\u88c1\u5207...","err");comp=srcImg;single=cropCanvas(srcImg,faceRes,srcImg,cropRect);}
  var fmt=document.querySelector("input[name=fmt]:checked").value,dpi=300,qual=parseInt(document.getElementById("qual").value);
  try{
    var resized=resize_image(single,parseInt(sz.value.split("x")[0])||295,parseInt(sz.value.split("x")[1])||413);
...
<div class="shell">
  ...
  <div class="workspace">
    ...
    <div class="panel" id="leftPanel">
      ...
      <div id="zoomCtrl" class="hidden" style="display:flex;gap:.35rem;align-items:center;margin-bottom:.5rem">
        <button id="zoomOut" style="border:none;background:var(--s2);border:1px solid var(--border);border-radius:8px;padding:.3rem .55rem;font-size:.72rem;cursor:pointer;color:var(--text);line-height:1">🔍-</button>
        <span style="font-size:.7rem;color:var(--muted);min-width:2.2rem;text-align:center" id="zoomPct">100%</span>
        <button id="zoomIn" style="border:none;background:var(--s2);border:1px solid var(--border);border-radius:8px;padding:.3rem .55rem;font-size:.72rem;cursor:pointer;color:var(--text);line-height:1">🔍+</button>
        <button id="zoomReset" style="border:none;background:var(--s2);border:1px solid var(--border);border-radius:8px;padding:.3rem .55rem;font-size:.72rem;cursor:pointer;color:var(--text);line-height:1;margin-left:.15rem">⟲</button>
      </div>
